...
**fallback**
엔드포인트에 올린 모델이 `429`/`5XX`로 실패하면 다음 대상으로 자동 재시도하는 기능. **legacy Serving endpoint**에서는 External models 전용이며 최대 2개까지 지정하고, 트래픽 `0%` 모델이 fallback 전용으로 동작합니다. **Unity AI Gateway**에서는 model service의 Routing 탭에서 우선순위를 직접 지정하며 개수 상한이 명시돼 있지 않고, Pay-per-token 모델도 destination으로 사용할 수 있습니다.
